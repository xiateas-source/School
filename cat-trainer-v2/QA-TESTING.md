...
| `ptoday-row` | A quest row on the Today board; carries `data-quest-id`. Its primary action is `[data-sirus-done]` — **Mark done** for an untouched quest (confirmed, since it credits minutes Sirus did not request) or **✓ Approve** for one he has already tapped; an approved quest shows only a chip. A block header carrying two or more unfinished quests also offers `[data-block-done]`, which marks that whole block done behind one confirm (already-approved quests are skipped, never re-credited). Its today-only scheduling actions are `[data-today-skip]`, `[data-today-move]` (Later), `[data-today-next]`, `[data-today-clear]` (Undo) |
...
| `weekly-goal-input`, `weekly-goal-save` | The week's goal, set from the parent Quests → Today screen. Saving an empty box clears it. Stored on the child profile, so it needs no rules change |
| `child-weekly-goal` | The read-only banner on Sirus's home screen. Hidden entirely when no goal is set. Deliberately inert — no points, no streak, nothing to tick — so it can never become another thing to fail |
| `quest-timer-mode` | Timer mode on the Add/Edit quest dialog: none, fixed duration, optional race, count up. Choosing a race on a quest the safety rules forbid (School daypart, brushing/eating/hot/sharp titles, or one marked never-race) shows a note and runs as a plain timer instead — it is not silently accepted |
| `timer-start`, `timer-live` | Start a Quest's timer and its running clock. Present on **both** Sirus's card and Mom's Today row, and only while the quest is unfinished. Device-local and unsynced, so a timer on one device never appears on the other — and a timer **never** gates completion: the Done / Mark done control behaves identically whether it ran, was ignored, or was stopped early |
